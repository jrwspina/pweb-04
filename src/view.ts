namespace View {
    ...
    export class TagsView extends TabView {
        ...
        render(items: ToDoItem[]): void {
            let tagsMap = new Map<string, ToDoItem[]>()
            for (const item of items) {
                if (item.tags) {
                    for (const tag of item.tags) {
                        if (tagsMap.has(tag)) {
            ...
            orderedTagsMap.forEach((value, tag) => {
                ...
                const template = document.querySelector("#tag-item-template") as HTMLTemplateElement
                const clone = template.content.cloneNode(true) as DocumentFragment
                const tagItem = clone.querySelector(".list-group-item")
                const badgeContainer = clone.querySelector(".badge-container")
                const badgeTemplate = badgeContainer?.querySelector(".tag-item-badge")
                ...
                if (tagItem) {
                    this.contentEl?.append(tagItem)
                }
                ordered_items.forEach(item => {
                    const listTemplate = document.querySelector("#group-item-template") as HTMLTemplateElement
                    const groupClone = listTemplate.content.cloneNode(true) as DocumentFragment
