...
(function (View) {
        ...
        render(items) {
            ...
            for (const item of items) {
                if (item.tags) {
                    for (const tag of item.tags) {
                        if (tagsMap.has(tag)) {
                            let currentItems = tagsMap.get(tag);
                            if (currentItems) {
                                tagsMap.set(tag, currentItems.concat([item]));
                            }
                            else {
                    ...
                    if (tagsMap.has("untagged")) {
                        let currentItems = tagsMap.get("untagged");
                        if (currentItems) {
                            tagsMap.set("untagged", currentItems.concat([item]));
                        }
                        else {
                            tagsMap.set("untagged", [item]);
                        }
                    }
                    else {
                        tagsMap.set("untagged", [item]);
            ...
            let orderedTagsMap = new Map([...tagsMap].sort());
            this.clearContainer();
            orderedTagsMap.forEach((value, tag) => {
                var _a, _b;
                let ordered_items = value;
                ordered_items.sort(this.compare);
                const template = document.querySelector("#tag-item-template");
                const clone = template.content.cloneNode(true);
                const tagItem = clone.querySelector(".list-group-item");
                const badgeContainer = clone.querySelector(".badge-container");
                const badgeTemplate = badgeContainer === null || badgeContainer === void 0 ? void 0 : badgeContainer.querySelector(".tag-item-badge");
                const newBadge = badgeTemplate === null || badgeTemplate === void 0 ? void 0 : badgeTemplate.cloneNode();
                newBadge.textContent = tag;
                badgeContainer === null || badgeContainer === void 0 ? void 0 : badgeContainer.append(newBadge);
                (_a = badgeTemplate === null || badgeTemplate === void 0 ? void 0 : badgeTemplate.parentElement) === null || _a === void 0 ? void 0 : _a.removeChild(badgeTemplate);
                if (tagItem) {
                    (_b = this.contentEl) === null || _b === void 0 ? void 0 : _b.append(tagItem);
                }
                ordered_items.forEach(item => {
                    var _a, _b;
                    const listTemplate = document.querySelector("#group-item-template");
                    const groupClone = listTemplate.content.cloneNode(true);
                    const listItem = groupClone.querySelector(".list-group-item");
                    const checkBox = groupClone.querySelector(".form-check-input");
                    const description = groupClone.querySelector(".list-item-desc");
                    const deadline = groupClone.querySelector(".list-item-deadline");
                    checkBox === null || checkBox === void 0 ? void 0 : checkBox.setAttribute("data-id", ((_a = item.id) === null || _a === void 0 ? void 0 : _a.toString()) || "");
                    if (description) {
                        description.textContent = item.description;
                    }
                    ...
                    if (listItem) {
                        (_b = this.contentEl) === null || _b === void 0 ? void 0 : _b.append(listItem);
                    }
                });
            });
        }
    ...
    class EditView {
        ...
        parse(id) {
            ...
            const updatedItem = new ToDoItem(descriptionEl.value);
            updatedItem.id = id;
            updatedItem.tags = tagsEl.value.split(",")
                .map(s => s.trim()).filter(s => s.length > 0);
            updatedItem.deadline = deadlineEl.value || "";
            return updatedItem;
        }
        /**
         *
        ...
        render(item) {
            ...
            if (item.deadline) {
                deadlineEl.valueAsDate = new Date(item.deadline);
            }
            else {
                deadlineEl.valueAsDate = null;
            }
            this.modalRef.show();
        }
        /**
